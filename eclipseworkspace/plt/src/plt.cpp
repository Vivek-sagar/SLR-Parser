...
/*
    ...
    for (int i=0; i < item_count; i++)
    {
        for (int j = 0; characters[j] != ' '; j++)
        {
            ...
            c[i].go2(characters[j], gotoprods, num_of_prods);
            for (int l=0; l<num_of_prods; l++)
            {
                cout << "\n Adding: " << gotoprods[i];
                c[item_count].add_prod(gotoprods[l]);
            }
            c[item_count].set_name("jj");//TODO
            for (int k=0; k<item_count; k++)
            {
                if (groups_are_different(c[item_count], c[k]))
                {
                    cout << "Item count incremented to " << item_count+1 << '\n';
                    item_count++;
                    break;
                }
            }
